...
class DataReader(object):
    ...
    def __init__(self, input_dir, output_dir, num_steps=10):
        ...
        self._num_samples = np.shape(self._outputs)[0]

        print("initialization is done.")
        print("current file number : %d" % (self._num_file + 1))
    ...
    def next_batch(self, batch_size):
        ...
        if self._start_idx + batch_size > self._num_samples:

            self._start_idx = 0
            #self._num_file += 1
            print("No more new data in this file. try to read other file to make a mini-batch")
            if self._num_file > self._file_len - 1:
                self._num_file = 0
                print("No more new file. try to return to first file to make a mini-batch")
            ...
            self._num_samples = np.shape(self._outputs)[0]
            print("current file number : %d, samples : %d" % (self._num_file + 1, self._num_samples))
